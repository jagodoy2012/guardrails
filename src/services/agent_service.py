...
from src.services.guardrails import blocked_answer, validate_input, validate_output
...
class AgentService:
    ...
    async def chat(self, question: str, user: str, session_id: Optional[str]) -> dict:
        ...
        input_guardrail = validate_input(question=question, user=user)
        if input_guardrail.blocked:
            logger.warning(
                "Solicitud bloqueada por guardrail",
                extra={
            ...
            return {
                "user": user,
                "answer": blocked_answer(input_guardrail),
                "session_id": session_id,
                "trace_id": trace_id,
            }
        ...
        output_guardrail = validate_output(result.get("answer"))
        if output_guardrail.blocked:
            ...
            result["answer"] = blocked_answer(output_guardrail)
